...
const TRACE_COV_MAX_INK_RATIO = 2.8; // 필기 면적 ÷ 글자 면적 상한(초과 시 낙서로 차단)

/* 완성 판정 난이도 — 설정에서 변경. recall/precision 임계를 조절한다(어린아이가
   많이 어려워하면 '쉽게', 또박또박 연습시키려면 '엄격'). 기본 '보통'. */
const TRACE_JUDGE_LEVELS = [
  { id: 'easy', label: '쉽게', recall: 0.75, precision: 0.45 },
  { id: 'normal', label: '보통', recall: TRACE_COV_RECALL_MIN, precision: TRACE_COV_PRECISION_MIN },
  { id: 'strict', label: '엄격', recall: 0.92, precision: 0.62 }
];
let _traceJudgeId = null;
function traceJudgeLevel() {
  if (_traceJudgeId == null) {
    const v = (typeof Utils !== 'undefined') ? Utils.loadLocal('tracing.judge.v1', 'normal') : 'normal';
    _traceJudgeId = TRACE_JUDGE_LEVELS.some((l) => l.id === v) ? v : 'normal';
  }
  return _traceJudgeId;
}
function traceSetJudgeLevel(id) {
  if (!TRACE_JUDGE_LEVELS.some((l) => l.id === id)) return;
  _traceJudgeId = id;
  if (typeof Utils !== 'undefined') Utils.saveLocal('tracing.judge.v1', id);
}
function _traceJudge() {
  const id = traceJudgeLevel();
  return TRACE_JUDGE_LEVELS.find((l) => l.id === id) || TRACE_JUDGE_LEVELS[1];
}
...
function _traceCoverageMaps(drawCanvas, target, opts) {
  ...
  // 숫자(0-9)는 폰트 글자에 세리프·굵은 슬랩이 있어 한 획으로 자연스럽게 그어도
  // 그 면적을 다 못 덮는다(특히 1·4·7) → 숫자는 항상 넉넉한 recall 반경을 준다.
  const allDigits = list.length > 0 && list.every((c) => /^[0-9]$/.test(c));
  const isThin = allDigits || thinSpan <= Math.max(4, 0.16 * Math.max(mw, mh));
...
function _traceCovFromMaps(maps, recallMin, precisionMin) {
  ...
  // 과도한 잉크 차단 — 글자 위를 빙빙 채우는 나선/원 낙서는 precision 이 임계를
  // 살짝 넘기도 하지만, 필기 면적이 글자 면적보다 훨씬 크다(복잡 음절에서 특히).
...
function traceEvaluateTracing(drawCanvas, target, opts) {
  opts = opts || {};
  const recallMin = typeof opts.recallMin === 'number' ? opts.recallMin : _traceJudge().recall;
  const precisionMin = typeof opts.precisionMin === 'number' ? opts.precisionMin : _traceJudge().precision;
