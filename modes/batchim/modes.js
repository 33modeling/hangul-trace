...
/* 기본 자모(COMMON.CHARS) 외 복합 모음·자모 이름 보조표 — '책'(ㅐ) 등 기본
   모음이 아닌 글자의 조합 타일 이름이 빈칸으로 보이는 문제 방지. */
const TRACE_JAMO_NAME_EXTRA = {
  'ㅐ': '애', 'ㅔ': '에', 'ㅒ': '얘', 'ㅖ': '예',
...
function _traceJamoName(ch) {
  ...
  if (TRACE_JAMO_NAME_EXTRA[ch]) return TRACE_JAMO_NAME_EXTRA[ch];
  return ch || '';
}
